          ...
          <div className="flex items-center gap-1">
            {user ? (<>
              {['/dashboard', '/items', '/ai'].map((to, i) => {
                ...
                return <Link key={to} to={to} className={`px-3 py-1.5 rounded-lg text-sm font-medium transition ${isActive(to) ? 'bg-white/20' : 'text-white/70 hover:text-white hover:bg-white/10'}`}>{labels[i]}</Link>
              })}
              {isAdmin && <Link to="/admin" className={`px-3 py-1.5 rounded-lg text-sm font-medium transition ${isActive('/admin') ? 'bg-white/20' : 'text-white/70 hover:text-white'}`}>Админ</Link>}
              <div className="ml-3 pl-3 border-l border-current/10 flex items-center gap-2">
                ...
                <button onClick={handleLogout} className="text-sm text-white/50 hover:text-white/80 px-2 py-1 rounded-lg hover:bg-white/10">Выход</button>
              </div>
            </>) : (<>
      ...
      <div className="md:hidden fixed inset-0 z-50">
        ...
        <div className="absolute bottom-16 left-4 right-4 bg-white rounded-2xl shadow-2xl border border-gray-100 p-4 space-y-2">
          ...
          <div className="border-t border-gray-100 pt-2 mt-2">
            <div className="px-3 py-1 text-xs text-white/50">{user?.username} · {user?.role}</div>
