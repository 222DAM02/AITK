...
from .services import generate_ai_response, fetch_finance_data
...
class AIGenerateView(APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            return Response({"detail": f"Ошибка AI: {str(e)}"}, status=status.HTTP_502_BAD_GATEWAY)
        query = AIQuery.objects.create(user=request.user, prompt=prompt, result=result["text"], sources=result["sources"])
...
class AISaveAsItemView(APIView):
    ...
    def post(self, request):
        ...
        serializer.is_valid(raise_exception=True)
        try:
            query = AIQuery.objects.get(id=serializer.validated_data['query_id'], user=request.user)
        except AIQuery.DoesNotExist:
            return Response({"detail": "Не найдено."}, status=status.HTTP_404_NOT_FOUND)
        item = Item.objects.create(owner=request.user, title=serializer.validated_data['title'], description=query.result, amount=0)
...
class KnowledgeBaseStatsView(APIView):
    ...
    def get(self, request):
        total = KnowledgeBase.objects.count()
        with_emb = KnowledgeBase.objects.exclude(embedding__isnull=True).count()
        cats = list(KnowledgeBase.objects.values_list('category', flat=True).distinct())
        return Response({"total": total, "with_embeddings": with_emb, "categories": cats})
