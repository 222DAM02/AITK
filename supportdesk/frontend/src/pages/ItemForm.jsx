...
export default function ItemForm() {
  ...
  const handleSubmit = async (rawData) => {
    setLoading(true)
    setError('')
    const numOrDateFields = config.entityFields.filter(f => f.type === 'number' || f.type === 'date').map(f => f.name)
    const formData = Object.fromEntries(
      Object.entries(rawData).map(([k, v]) => [k, v === '' && numOrDateFields.includes(k) ? null : v])
    )
    try {
      if (isEdit) {
