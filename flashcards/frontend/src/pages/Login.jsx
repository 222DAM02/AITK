    ...
    <div className="min-h-[80vh] flex items-center justify-center px-4 py-12">
      <div className="w-full max-w-md">
        <div className="text-center mb-8">
          ...
          <h1 className="text-2xl font-bold text-gray-100">Вход</h1>
          <p className="text-gray-500 mt-2 text-sm">Войдите в {config.projectName}</p>
        </div>
        <div className={`${s.surface} rounded-2xl p-8`}>
          {location.state?.registered && (
            <div className="bg-emerald-500/10 text-emerald-400 p-3 rounded-xl mb-5 text-sm border border-emerald-500/20">
        ...
        <p className="text-center mt-6 text-gray-500 text-sm">
          Нет аккаунта? <Link to="/register" className={`${s.accent} font-semibold ${s.accentHover}`}>Зарегистрироваться</Link>
        </p>
      </div>
    </div>
  )
