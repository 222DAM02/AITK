...
from .services import generate_ai_response, fetch_productivity_data
...
class AIGenerateView(APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            return Response({"detail": f"Ошибка AI сервиса: {str(e)}"}, status=status.HTTP_502_BAD_GATEWAY)
...
class AISaveAsItemView(APIView):
    ...
    def post(self, request):
        ...
        item = Item.objects.create(owner=request.user, title=serializer.validated_data['title'], description=query.result)
...
class KnowledgeBaseStatsView(APIView):
    ...
    def get(self, request):
        ...
        return Response({"total": total, "with_embeddings": with_embeddings, "categories": categories})
