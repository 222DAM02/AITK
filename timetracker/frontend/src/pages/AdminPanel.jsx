  ...
  return (
    <div className="max-w-5xl mx-auto px-4 sm:px-6 py-10">
      ...
      <div className="flex gap-2 mb-6">{['users','items'].map(t=>(<button key={t} onClick={()=>setTab(t)}
        ...
        {t==='users'?'Пользователи':'Записи'}</button>))}</div>
      {tab==='users'&&(<div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden"><table className="w-full"><thead className="bg-gray-50">
      ...
      {tab==='items'&&(<div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden"><table className="w-full"><thead className="bg-gray-50">
        <tr><th className="text-left px-5 py-3 text-xs font-medium text-gray-500 uppercase">Название</th>
          <th className="text-left px-5 py-3 text-xs font-medium text-gray-500 uppercase">Автор</th><th className="px-5 py-3"></th></tr></thead>
