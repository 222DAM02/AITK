...
from .services import generate_ai_response, fetch_holidays_data
...
class AIGenerateView(APIView):
    ...
    def post(self, request):
        ...
        try: result = generate_ai_response(prompt)
        except Exception as e: return Response({"detail": str(e)}, status=status.HTTP_502_BAD_GATEWAY)
        q = AIQuery.objects.create(user=request.user, prompt=prompt, result=result["text"], sources=result["sources"])
...
class AISaveAsItemView(APIView):
    permission_classes = [IsAuthenticated]
    def post(self, request):
        ser = AISaveAsItemSerializer(data=request.data)
        ser.is_valid(raise_exception=True)
        try: q = AIQuery.objects.get(id=ser.validated_data['query_id'], user=request.user)
        except AIQuery.DoesNotExist: return Response({"detail": "Не найдено."}, status=status.HTTP_404_NOT_FOUND)
        item = Item.objects.create(owner=request.user, title=ser.validated_data['title'], description=q.result)
        return Response({"detail": "Сохранено.", "item_id": item.id}, status=status.HTTP_201_CREATED)

class FetchDataView(APIView):
    permission_classes = [IsAuthenticated]
    def post(self, request):
        try: count = fetch_holidays_data()
