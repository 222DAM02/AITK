...
export default function AIPage() {
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 py-8">
      ...
      {/* KB Stats */}
      <div className="bg-gradient-to-r from-lime-50 to-emerald-50 rounded-2xl p-5 border border-lime-100 mb-6">
        <div className="flex items-center justify-between flex-wrap gap-3">
          <div className="flex items-center gap-6">
            <div>
              <p className="text-xs text-lime-500 font-medium uppercase tracking-wider">База знаний</p>
              <p className="text-2xl font-bold text-lime-700">{kbStats?.total || 0} <span className="text-sm font-normal text-lime-400">записей</span></p>
            </div>
            <div>
              <p className="text-xs text-lime-500 font-medium uppercase tracking-wider">С эмбеддингами</p>
              <p className="text-2xl font-bold text-lime-700">{kbStats?.with_embeddings || 0}</p>
            </div>
          </div>
          <button onClick={handleFetchData} disabled={fetching}
            ...
            {fetching ? <><span className="animate-spin h-4 w-4 border-2 border-white border-t-transparent rounded-full" /> Загрузка...</> : '📥 Загрузить упражнения из Wger API'}
          </button>
        </div>
      </div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {sources.map((src, i) => (
                      <a key={i} href={src.source} target="_blank" rel="noopener noreferrer"
                        className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-lime-50 text-lime-700 rounded-lg text-xs font-medium hover:bg-lime-100 transition">
                        🔗 {src.title}
                      </a>
                    ))}
                  </div>
                  ...
                  <li key={q.id} className="py-3 cursor-pointer hover:bg-gray-50 rounded-lg px-2 -mx-2 transition"
                    ...
                    {q.sources?.length > 0 && (
                      <div className="flex gap-1 mt-1">
                        {q.sources.map((src, i) => (
                          <span key={i} className="text-xs bg-lime-50 text-lime-600 px-2 py-0.5 rounded-full">{src.title}</span>
                        ))}
                      </div>
                    )}
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
          ...
          {knowledge.length === 0 ? (
            ...
            <div className="space-y-3">
              {knowledge.map(kb => (
                <div key={kb.id} className="border border-gray-100 rounded-xl p-4 hover:border-lime-200 transition">
                  ...
                  {kb.source && <a href={kb.source} target="_blank" rel="noopener noreferrer"
                    className="text-xs text-lime-500 hover:text-lime-700 mt-2 inline-block">🔗 {kb.source}</a>}
                </div>
              ))}
            </div>
          )}
