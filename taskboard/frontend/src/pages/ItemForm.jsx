...
export default function ItemForm() {
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 sm:px-6 py-8">
      <Link to="/board" className="text-slate-500 hover:text-slate-300 text-sm mb-4 inline-block transition-colors">
        ← Назад к доске
      </Link>

      <h1 className="text-2xl font-bold text-white mb-6">
        {isEdit ? `Редактировать ${config.entityName.toLowerCase()}` : `Новая ${config.entityName.toLowerCase()}`}
      </h1>
