...
export default function Navbar() {
  ...
  return (
    <>
      {/* Mobile top bar */}
      <div className="lg:hidden flex items-center justify-between h-14 px-4 border-b border-current/10 sticky top-0 z-50 bg-inherit">
      ...
      {mobileOpen && (
        <div className="lg:hidden fixed inset-0 z-50 flex">
          ...
          <div className="relative bg-white w-64 shadow-2xl border-r border-gray-100" style={{background: 'inherit'}}>
            {sidebar}
          </div>
        </div>
      )}
      ...
      {/* Desktop sidebar */}
      <div className={`hidden lg:flex flex-col fixed left-0 top-0 h-screen border-r border-current/10 z-40 bg-inherit ${collapsed ? 'w-16' : 'w-56'} transition-all`}>
        {sidebar}
      </div>
