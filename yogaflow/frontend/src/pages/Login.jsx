    ...
    <div className="min-h-[80vh] flex items-center justify-center px-4 py-12">
      <div className="w-full max-w-md">
        <div className="text-center mb-8">
          ...
          <p className="text-gray-500 mt-1 text-sm">{config.projectName}</p>
        </div>
        <div className="bg-white rounded-2xl shadow-sm border border-stone-100 p-8">
          {location.state?.registered && (
            <div className="bg-emerald-50 text-emerald-700 p-3 rounded-xl mb-5 text-sm border border-emerald-100">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <button
              type="submit"
              disabled={loading}
              className={`w-full ${s.btnPrimary} py-3 rounded-xl disabled:opacity-50 text-base`}
            >
              {loading ? 'Вход...' : 'Войти'}
            </button>
          </form>
        ...
        <p className="text-center mt-5 text-gray-500 text-sm">
          Нет аккаунта?{' '}
          <Link to="/register" className={`${s.accent} font-semibold`}>Зарегистрироваться</Link>
        </p>
      </div>
    </div>
  )
