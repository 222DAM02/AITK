...
export default function DynamicForm({ initialData = {}, onSubmit, loading = false }) {
  const [formData, setFormData] = useState(() => {
    const data = {}
    config.entityFields.forEach((f) => { data[f.name] = initialData[f.name] || '' })
    return data
  })
  ...
  const validate = () => {
    const errs = {}
    config.entityFields.forEach((f) => {
      if (f.required && !formData[f.name]?.toString().trim()) errs[f.name] = 'Обязательное поле'
    })
    setErrors(errs)
        ...
        <div key={field.name}>
          ...
          {field.type === 'textarea' ? (
            <textarea value={formData[field.name]} onChange={(e) => handleChange(field.name, e.target.value)}
              placeholder={`Введите ${field.label.toLowerCase()}`} className={inputClass} rows={4} />
          ) : field.type === 'select' ? (
