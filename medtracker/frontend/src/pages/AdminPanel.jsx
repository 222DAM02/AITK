      ...
      <div className="flex gap-2 mb-5">
        {[['users', `Пользователи (${users.length})`], ['items', `${config.entityNamePlural} (${items.length})`]].map(([name, label]) => (
          <button key={name} onClick={() => setTab(name)}
            className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${tab === name ? s.btnPrimary : 'bg-white text-gray-600 border border-gray-200 hover:bg-gray-50'}`}>
            {label}
          </button>
        ))}
      </div>
      ...
      <div className="bg-white rounded-xl border border-gray-100 overflow-hidden">
        <div className="overflow-x-auto">
          {tab === 'users' ? (
                ...
                {users.map(u => (
                  <tr key={u.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3">
                      <span className={`text-xs px-2 py-0.5 rounded font-medium ${u.role === 'admin' ? 'bg-purple-100 text-purple-700' : s.badge}`}>{u.role}</span>
                    </td>
                    <td className="px-4 py-3">
            ...
            <table className="w-full text-left">
              <thead><tr className="bg-gray-50 border-b border-gray-100">
                {['ID', 'Препарат', 'Дозировка', 'Владелец', 'Дата', 'Действия'].map(h => (
                  <th key={h} className="px-4 py-3 text-xs uppercase tracking-wider text-gray-400 font-semibold">{h}</th>
                ))}
              </tr></thead>
              <tbody className="divide-y divide-gray-50">
